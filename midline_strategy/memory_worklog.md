...
| 阶段 | 内容 | 状态 |
|:----|:----|:----:|
| P0 | 统一特征管道 (feature_pipeline.py) | ✅ |
| P0a | 向量化训练 (build_training_dataset) | ✅ |
| P1 | 精简特征 26→24 | ✅ |
| P2 | 重训 lgb_midline (AUC 0.611) | ✅ |
| P3 | 元标注训练 (AUC 0.579) | ✅ |
| P4 | 软集成上线 (main*0.7 + main*meta*0.3) | ✅ |
| 审核修复 | pipeline 返回排序 bug + 特征计算不一致 | ✅ |
| 审核修复 | paper_signals 表结构统一 + meta_score 持久化 | ✅ |
| 回测 | 规则 vs 规则+LGB (2018-2020 3年对比) | ✅ |
...
**核心结论:** LGB 在所有指标上优于纯规则，但年化绝对值低(0.56%→0.87%)。回测数据可信度有限（训练-回测区间重叠）。线上真实数据才是最终验证。

### 系统状态

- v3.2 在本地运行正常
- 日报推送正常（Telegram + Server酱）
- 虚拟盘持仓 2 只: 000404 长虹华意, 002906 华阳集团
- lgb_score / meta_score 正确写入 paper_signals 表

### 下一步（不执行，仅记录）

- 让系统自主运行，每日看日报
- 积累 50+ 笔实盘交易后评估模型效果
- 趋势通道研究（P1）: 等待实盘数据积累后启动
